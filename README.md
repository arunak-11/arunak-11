#👋 Hi, I'm Arun — Cloud & DevOps Engineer

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/arunks11)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:arunsubramaniak@gmail.com)

I build **secure, scalable cloud infrastructure on AWS** using **Infrastructure-as-Code**, **CI/CD automation**, and **containerization** for FinTech, Healthcare, and ERP SaaS applications.

🔧 **Tech Stack**:  
![AWS](https://img.shields.io/badge/AWS-232F3E?logo=amazon-aws&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-7B42BC?logo=terraform&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?logo=jenkins&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?logo=node.js&logoColor=white)

---
🚀 Featured Projects

| Project | Domain | Tech Used | Description |

|--------|--------|----------|-------------|

| **[CRM Application]**

        ERP-Cloud | AWS, Python, MySQL, Terraform | Cloud-hosted CRM for subscriber & support management |
        
| **[HCM System]** 

         Healthcare | AWS Lambda, Docker, VPC, IAM | End-to-end healthcare platform with OPD, IPD, Blood Bank |
         
| **[FIM Platform]** 

         FinTech | AWS CodeBuild, Jenkins, Boto3 | Finance & investment tool with budgeting and expense tracking |

> 🔜 #https://github.com/arunak-11/arunak-11.git

---

### 🛠️ DevOps Practices I Follow
- ✅ Infrastructure as Code (Terraform > CloudFormation)
- ✅ Automated CI/CD pipelines (Jenkins, GitHub Actions, AWS CodeBuild)
- ✅ Secure cloud design (IAM, VPC, TLS, Least Privilege)
- ✅ Scripting for automation (Python/Boto3, Shell)
- ✅ Observability via CloudWatch & log-driven debugging

---

 📜 Certifications
- Cloud Computing Architecture (Jan 2024)
- Power BI (Nov 2024)
- Online Cloud Computing (2023)

💬 **Open to DevOps / Cloud Engineering roles (Remote or India)**  

---

📫 Reach out & Connect with Me🌟

🌐[LinkedIn](https://www.linkedin.com/in/arunks11) or 
💬arumsubramaniak@gmail.com



